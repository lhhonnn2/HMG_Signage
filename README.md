...
버전에 따라 아래 중 하나만 실행하면 됩니다. 이미 `migration_v2.sql`까지 실행한 적이 있다면 `migration_v3.sql`만 추가로 실행하세요.

- 처음 설치: `schema.sql`만 실행
- 아주 처음 버전만 실행해봤던 경우: `migration_v2.sql` → `migration_v3.sql` 순서로 실행
- `migration_v2.sql`까지는 이미 실행해봤던 경우: `migration_v3.sql`만 실행
