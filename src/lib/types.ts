...
export type DayPlanRow = {
  id: string;
  name: string;
  rows: { program_name: string; location: string; start_time: string }[];
};
...
// Picks readable black or white text for a given background hex color, so
// a custom (possibly light) background doesn't end up with invisible
// white-on-white text. Shared with the player's AlarmView.
